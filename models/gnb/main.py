...
for fold, (train_idx, test_idx) in enumerate(kf.split(image_ids, labels)):
    ...
    gnb_hardness = clone(clf)
    gnb_honey = clone(clf)
    gnb_capped = clone(clf)
    ...
    for task, model, train_labels, test_labels, task_name in [
        (gnb_hardness, "hardness", train_labels_hardness, test_labels_hardness, "hardness"),
        (gnb_honey, "honey", train_labels_honey, test_labels_honey, "honey"),
        (gnb_capped, "capped", train_labels_capped, test_labels_capped, "capped"),
    ]:
        ...
        model_path = os.path.join(SAVE_DIR, f"gnb_{task_name}_fold_{fold + 1}.joblib")
